/* $Id: plctrl.c 9727 2009-03-12 08:18:31Z smekal $
...
plabort(const char *errormsg)
{
	...
	*(plsc->errcode) = 1;
    ...
    if (plsc->errmsg != NULL) {
	snprintf(plsc->errmsg, 256, "\n*** PLPLOT ERROR, ABORTING OPERATION ***\n");
	if (*errormsg != '\0')
	    snprintf(plsc->errmsg, 256 , "%s, aborting operation\n", errormsg);
...
plsnscanf( const char *buffer, int n, const char *format, ... )
{
  ...
  va_start(argptr, format);
	ret=vsscanf(buffer, format, argptr);
  va_end(argptr);
