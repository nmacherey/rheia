...
if( macosx ) then
    CP="cp -r "
    package.config["Debug"].links = { "gmcsirocsa-dbg", "gmqsastime-dbg", "gmwxwidgets" }
	package.config["Release"].links = { "gmwxwidgets", "gmcsirocsa", "gmqsastime" }
else
    CP="cp -ru "
end
...
if ( not windows ) then
	package.files = { matchfiles( "../wxwidgets/*.cpp", "../wxwidgets/*.hpp", "../wxwidgets/*.cxx", "../wxwidgets/*.h", "../wxwidgets/*.cc", "../wxwidgets/*.hh" , "../wxwidgets/*.c" ) , matchfiles( "*.cpp", "*.hpp", "*.cxx", "*.h", "*.cc", "*.hh" , "*.c" ), matchfiles( "../../include/plplot/*.h" ) }
else
	package.files = { matchfiles( "../wxwidgets/*.cpp", "../wxwidgets/*.hpp", "../wxwidgets/*.cxx", "../wxwidgets/*.h", "../wxwidgets/*.cc", "../wxwidgets/*.hh" , "../wxwidgets/*.c" ) , matchrecursive( "*.cpp", "*.hpp", "*.cxx", "*.h", "*.cc", "*.hh" , "*.c" ) , matchrecursive( "../../include/plplot/*.h" ) }
end
...
--*********************************
	if ( (not options["cb-msvc8"]) and (string.find( target or "", ".*-gcc" ) or target == "gnu") ) then
		...
		package.config["Release"].postbuildcommands = { "gmkdir -p ../../devel/Release/include/rheia" , "cp -ru ../../include/plplot ../../devel/Release/include/rheia" }
		package.config["Debug"].postbuildcommands = { "gmkdir -p ../../devel/Debug/include/rheia" , "cp -ru ../../include/plplot ../../devel/Debug/include/rheia" }
	else
		...
		table.insert( package.buildoptions, "/Dplplotd_EXPORTS" )
		table.insert( package.buildoptions, "/Dwxwidgets_EXPORTS" )
	...
	package.config["Debug"].links = { "libgmcsirocsa-dbg", "libgmqsastime-dbg" }
	package.config["Release"].links = { "libgmcsirocsa", "libgmqsastime" }
