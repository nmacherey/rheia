...
if ( not windows ) then
	package.files = { matchfiles( "*.cpp", "*.hpp", "*.cxx", "*.h", "*.cc", "*.hh" , "*.c" ), matchfiles( "../../include/wxplplot/*.h" ) }
else
	package.files = { matchrecursive( "*.cpp", "*.hpp", "*.cxx", "*.h", "*.cc", "*.hh" , "*.c" ) , matchrecursive( "../../include/wxplplot/*.h" ) }
end
...
--*********************************
	if ( (not options["cb-msvc8"]) and (string.find( target or "", ".*-gcc" ) or target == "gnu") ) then
		...
		package.config["Release"].postbuildcommands = { "gmkdir -p ../../devel/Release/include/rheia" , "cp -ru ../../include/wxplplot ../../devel/Release/include/rheia" }
		package.config["Debug"].postbuildcommands = { "gmkdir -p ../../devel/Debug/include/rheia" , "cp -ru ../../include/wxplplot ../../devel/Debug/include/rheia" }
	else
	...
	package.config["Debug"].links = { "libgmcsirocsa-dbg", "libgmqsastime-dbg" , "libgmplplot-dbg" }
	package.config["Release"].links = { "libgmcsirocsa", "libgmqsastime" , "libgmplplot" }
