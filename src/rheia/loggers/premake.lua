...
package.name = "rheialoggers"

-- Set this if you want a different name for your target than the package's name.
local targetName = "rheialoggers"
local version = "1.1.1"
...
if( macosx ) then
    CP="cp -r "
    package.config["Debug"].links = { "rheiautils-dbg" , "rheiabase-dbg"}
	package.config["Release"].links = { "rheiautils" , "rheiabase" }
else
    CP="cp -ru "
end
...
if ( not windows ) then
	package.files = { matchfiles( "*.cpp", "*.hpp", "*.cxx", "*.h", "*.cc", "*.hh" , "*.c" ), matchfiles( "../../../include/rheia/loggers/*.h" ) }
else
	package.files = { matchrecursive( "*.cpp", "*.hpp", "*.cxx", "*.h", "*.cc", "*.hh" , "*.c" ) , matchrecursive( "../../../include/rheia/loggers/*.h" ) }
end

-- Set the include paths.
package.includepaths = { "../../../include/rheia/loggers" , "../../../include/rheia/base" , "../../../include/rheia/utils" , "../../../include/irrlicht" , "../../../src/irrlicht" }

-- Set the packages dependancies. NOT implimented in the official Premake build for Code::Blocks
package.depends = { "csirocsa", "qsastime" , "plplot" , "irrlicht" , "rheiautils" , "rheiabase" }

-- Set the defines.
package.defines = { "HAVE_CONFIG_H", "RHEIA_LOGGERS_MAKINGDLL" , "RHEIA_USE_IRRLICHT" }
...
--*********************************
	...
	if ( (not options["cb-msvc8"]) and (string.find( target or "", ".*-gcc" ) or target == "gnu") ) then
		...
		package.config["Release"].postbuildcommands = { "gmkdir -p ../../../devel/Release/include/rheia" , "cp -ru .../../../include/rheia/loggers ../../../devel/Release/include/rheia" }
		package.config["Debug"].postbuildcommands = { "gmkdir -p ../../../devel/Debug/include/rheia" , "cp -ru ../../../include/rheia/loggers ../../../devel/Debug/include/rheia" }
	else
		...
		table.insert( package.buildoptions, "/DRHEIA_LOGGERS_MAKINGDLL" )
	...
	package.config["Debug"].links = { "libxml2-dbg" , "libgmirrlicht-dbg", "libgmcsirocsa-dbg", "libgmqsastime-dbg" , "libgmplplot-dbg" , "librheiautils-dbg" , "librheiabase-dbg" }
	package.config["Release"].links = { "libxml2" , "libgmirrlicht" , "libgmcsirocsa", "libgmqsastime" , "libgmplplot" , "librheiautils" , "librheiabase" }
