...
if( macosx ) then
    CP="cp -r "
    package.config["Debug"].links = { "rheiautils-dbg" , "rheiabase-dbg" , "rheialoggers-dbg" }
	package.config["Release"].links = { "rheiautils" , "rheiabase" , "rheialoggers" }
else
    CP="cp -ru "
end
...
if ( not windows ) then
	package.files = { matchfiles( "*.cpp", "*.hpp", "*.cxx", "*.h", "*.cc", "*.hh" , "*.c" ), matchfiles( "../../../include/rheia/packagemgt/*.h" ) }
else
	package.files = { matchrecursive( "*.cpp", "*.hpp", "*.cxx", "*.h", "*.cc", "*.hh" , "*.c" ) , matchrecursive( "../../../include/rheia/packagemgt/*.h" ) }
end

-- Set the include paths.
package.includepaths = { "../../../include/rheia/packagemgt" , "../../../include/rheia/loggers" , "../../../include/rheia/base" , "../../../include/rheia/utils" , "../../../include/irrlicht" , "../../../src/irrlicht" }
...
-- Set the defines.
package.defines = { "HAVE_CONFIG_H", "RHEIA_PMGT_MAKINGDLL" , "RHEIA_USE_IRRLICHT" }
...
--*********************************
	...
	if ( (not options["cb-msvc8"]) and (string.find( target or "", ".*-gcc" ) or target == "gnu") ) then
		...
		package.config["Release"].postbuildcommands = { "gmkdir -p ../../../devel/Release/include/rheia" , "cp -ru .../../../include/rheia/packagemgt ../../../devel/Release/include/rheia" }
		package.config["Debug"].postbuildcommands = { "gmkdir -p ../../../devel/Debug/include/rheia" , "cp -ru ../../../include/rheia/packagemgt ../../../devel/Debug/include/rheia" }
	else
		...
		table.insert( package.buildoptions, "/DRHEIA_PMGT_MAKINGDLL" )
	...
	package.config["Debug"].links = { "libxml2-dbg" , "libgmirrlicht-dbg", "libgmcsirocsa-dbg", "libgmqsastime-dbg" , "libgmplplot-dbg" , "librheiautils-dbg" , "librheiabase-dbg" , "librheialoggers-dbg" }
	package.config["Release"].links = { "libxml2" , "libgmirrlicht" , "libgmcsirocsa", "libgmqsastime" , "libgmplplot" , "librheiautils" , "librheiabase" , "librheialoggers" }
