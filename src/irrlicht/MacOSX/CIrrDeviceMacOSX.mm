...
bool CIrrDeviceMacOSX::createWindow()
{
	...
	if (!CreationParams.Fullscreen)
	{
		...
		if (_window != NULL || CreationParams.WindowId)
		{
			index = 0;
			...
			if (_oglcontext != NULL)
			{
				if(!CreationParams.WindowId)
				{
					...
					[(NSWindow *)_window setAcceptsMouseMovedEvents:FALSE];
					[(NSWindow *)_window setIsVisible:TRUE];
					[(NSWindow *)_window makeKeyAndOrderFront:nil];
				}
				else //use another window for drawing
					[(NSOpenGLContext *)_oglcontext setView:(NSView*) CreationParams.WindowId];
	...
	if (result)
	{
		if (_window == NULL && !CreationParams.WindowId) //hide menus in fullscreen mode only
...
{	
	...
	getVideoDriver()->OnResize(core::dimension2d<s32>( (s32)width, (s32)height));  //change
}
...
void CIrrDeviceMacOSX::setResizeAble(bool resize)
{
	...
	// (HACK) update the size of the opengl rendering context..we have to call this method everytime window resize..have to find another location
	if(CreationParams.WindowId && _oglcontext)
		[(NSOpenGLContext *)_oglcontext update];
